## Resource: `gigamon_anycloud_connection`

A Third‑Party Orchestration (3PO) Connection attaches a connection to a Third‑Party Orchestration Monitoring Domain in Gigamon Fabric Manager.  
...
```hcl
resource "gigamon_anycloud_connection" "terraform_conn" {
  alias                = "example-conn"
  monitoring_domain_id = gigamon_anycloud_monitoring_domain.terraform_md.id
  tapping_method       = gigamon_anycloud_monitoring_domain.terraform_md.tapping_method
}
```
...
```hcl
import {
  to = gigamon_anycloud_connection.terraform_conn
  id = "example-conn"
}
```
