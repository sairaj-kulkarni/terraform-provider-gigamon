...
---
page_title: "Endpoint Interface Mapping"
subcategory: "Endpoint Interface Mapping"
description: "Manage endpoint interface mappings in Gigamon FM."
---
...
1. Validates configuration:
    ...
    - At least one `mapping` block must be present (enforced by a config validator).
2. Builds the FM payload:
   ...
   ```
    - `monitoringSessionId` is the **raw UUID** derived from `monitoring_session_id`.  
...
5. Performs a best‑effort **read‑back** (`GET` to the same endpoint) and updates `vseries_node_ids` and `mapping` blocks from FM’s current configuration.
...
**the provider:**
1. Re‑builds the FM payload from the **planned** configuration (same shape as for creation).  
2. Issues a **POST** to `/endpointIfaceMappings`, which acts as an **upsert** for the mappings of the selected vSeries nodes.  
